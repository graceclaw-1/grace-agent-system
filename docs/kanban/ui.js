// ============================================================
// GRACE COMMAND CENTER — UI Interactions
// ============================================================
...
class KanbanUI {
  ...
  openModal(taskId = null, defaults = {}) {
    ...
    const task = taskId ? this.app.tasks.find(t => t.id === taskId) : null;
    ...
    if (agentSel) {
      agentSel.innerHTML = AGENTS.map(a =>
        `<option value="${a.id}" ${(task?.agentId || defaults.agentId) === a.id ? 'selected' : ''}>${a.emoji} ${a.name} — ${a.role}</option>`
      ).join('');
    }
    ...
    if (colSel) {
      colSel.innerHTML = COLUMNS.map(c =>
        `<option value="${c.id}" ${(task?.column || defaults.column || 'backlog') === c.id ? 'selected' : ''}>${c.icon} ${c.label}</option>`
      ).join('');
    }
  ...
  _handleFormSubmit() {
    ...
    if (this.editingTaskId) {
      this.app.editTask(this.editingTaskId, { title, description, agentId, column, priority });
    } else {
      this.app.addTask({ title, description, agentId, column, priority });
    }
  ...
  renderFlatView(tasks) {
      ...
      card.innerHTML = `
        <div class="flat-agent-header">
          ...
          <div>
            ...
            <div class="flat-agent-role">${agent.role}</div>
          </div>
    ...
    container.querySelectorAll('.flat-add-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        this.openModal(null, { agentId, column: 'backlog' });
      });
    });
    ...
    return `
      <div class="flat-card" draggable="true" data-task="${task.id}" data-agent="${task.agentId}" data-col="${task.column}"
        ...
        <div class="flat-card-body">
          ...
          ${task.description ? `<div class="flat-card-desc">${this._escapeHtml(task.description.substring(0, 80))}${task.description.length > 80 ? '…' : ''}</div>` : ''}
  ...
  _setupDragDrop(container) {
    ...
    container.querySelectorAll('.flat-col-tasks').forEach(colEl => {
      ...
      colEl.addEventListener('drop', (e) => {
        ...
        const id = colEl.id; // flat-col-grace-backlog
  ...
  showAgentSidebar(agent, tasks) {
    ...
    sidebar.innerHTML = `
      <div class="sidebar-header" style="border-color: ${agent.color}">
        ...
        <div class="sidebar-status status-${agent.status}">
      ...
      <div class="sidebar-stats">
        ${COLUMNS.map(c => `
          <div class="sidebar-stat">
            <div class="sidebar-stat-num" style="color: ${agent.color}">${stats[c.id]}</div>
                ...
                return `
                  <div class="sidebar-task-item" data-task="${t.id}">
                    ...
                    <div class="sidebar-task-actions">
                      <button class="flat-card-edit sidebar-edit-btn" data-task="${t.id}" title="Edit">✎</button>
                      <button class="flat-card-del sidebar-del-btn" data-task="${t.id}" title="Delete">✕</button>
                    </div>
                  </div>
                `;
              }).join('')}
